...
try {
  ...
  $resposta = $_POST['queryResposta'];
  $pesquisaResposta = $respostausuario->pesquisaResposta($resposta);
} catch (Exception $e) {
  echo $e->getMessage();
}
?>
...
<body>
  ...
  <header id="header" style="background-color: #5A56E9;" class="header fixed-top d-flex align-items-center">
    ...
    <div class="search-bar">
      <form class="search-form d-flex align-items-center" method="POST" action="./pesquisa-resposta.php">
        <input type="text" name="queryResposta" placeholder="Id ou Doador" title="Enter search keyword">
        <button type="submit" title="Search"><i class="bi bi-search"></i></button>
      </form>
    </div><!-- End Search Bar -->

    <form class="d-flex" role="search" action="./gera-relatorio-resposta.php" method="post">
    ...
    <div class="container-fluid">
      <div id="container">
        <table>
          ...
          <tbody>
            <?php foreach ($pesquisaResposta as $key => $listar) { ?>
              <tr>

                <td><?php echo $idRespostaUsuario = $listar['idrespostausuario'] ?></td>
                <td><?php echo $simOuNao = $listar['simounao'] ?></td>
                <td><?php echo $nomeDoador = $listar['nomedoador'] ?></td>
                <td><?php echo $descItem = $listar['descItem'] ?></td>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="./editar-user-resposta.php" method="post">
              ...
              <div class="form-group">
                <label for="recipient-resposta" class="col-form-label">RESPOSTA</label>
                <input type="text" class="form-control" id="recipient-resposta" name="respostaEditar">
              </div>
              <div class="form-group">
                <label for="recipient-doador" class="col-form-label">DOADOR</label>
                <input type="text" class="form-control" id="recipient-doador" name="doadorEditar">
              </div>
              <div class="form-group">
                <label for="recipient-desc" class="col-form-label">ITEM</label>
    ...
    <div class="modal fade" id="excluir" tabindex="-1" role="dialog" aria-labelledby="editar" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <form action="./decisao-excluir-resposta.php" method="post">
              <button type="button" class="btn btn-primary" data-dismiss="modal">CANCELAR</button>
              <button type="submit" class="btn btn-danger" name="idResposta" id="idResposta" value="<?php echo $idRespostaUsuario ?>">EXCLUIR</button>
            </form>
          </div>
        </div>
      </div>
  ...
  <script type="text/javascript">
    $('#excluir').on('show.bs.modal', function(event) {
      ...
      modal.find('#idResposta').val(recipient)
    ...
    $('#editar').on('show.bs.modal', function(event) {
      ...
      var recipient = button.data('whatever')
      var recipientResposta = button.data('whatever01')
      var recipientDoador = button.data('whatevernomedoador')
      ...
      modal.find('#recipient-resposta').val(recipientResposta)
      modal.find('#recipient-doador').val(recipientDoador)
