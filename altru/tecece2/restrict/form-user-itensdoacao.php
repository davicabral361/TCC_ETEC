<?php
require_once("../model/Doacao.php");

try{
    $doacao = new Doacao();

    $listadoacao = $doacao->listar();
} catch(Exception $e) {
    echo $e->getMessage();
}
?>

<!DOCTYPE html>
<html lang="pt-br">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Formulário de cadastro</title>
    <link rel="stylesheet" href="../css/form-user.css">
</head>

<body>

    <div class="box">
        <div class="img-box">
            <img src="../image/donate.png">
        </div>
        <div class="form-box">
            <form method="post" action="../restrita/cadastra-itensdoacao.php">
                <div class="input-group">
                    <label for="nome">Nome da Doação</label>
                    <input type="text" id="txtNomeDoacao" name="txtNomeDoacao" placeholder="Digite o nome da doação" required>
                </div>

                <div class="input-group">
                    <label for="itens">Quantidade de Itens</label>
                    <input type="number" id="txtQuantidadeItensDoacao" name="txtQuantidadeItensDoacao" placeholder="Digite a quantidade de itens" required>
                </div>

                <label>Doação:</label>
            <select name="doacao">
                <option value="0">Selecione</option>
                <?php foreach ($listadoacao as $listar){ ?>
                    <option value="<?php echo $listar['iddoacao'] ?>">
                        <?php echo $listar['iddoacao'] ?>
                    </option>
                <?php } ?>
            </select>

                <div class="input-group">
                    <button type="submit">Doe</a></button>
                </div>
                <p class="temConta">Já tem uma conta? <a href="login-user.php">LOGIN</a></p>
            </form>
        </div>
    </div>

</body>

</html>